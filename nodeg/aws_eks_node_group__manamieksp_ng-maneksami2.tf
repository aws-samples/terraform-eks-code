# File generated by aws2tf see https://github.com/awsandy/aws2tf

resource "aws_eks_node_group" "ng-experiment2" {
  #ami_type       = "AL2_x86_64"
  depends_on     = [aws_launch_template.lt-ng-experiment2]
  cluster_name   = data.aws_eks_cluster.eks_cluster.name
  disk_size      = 0
  instance_types = []
  labels = {
    "alpha.eksctl.io/cluster-name"   = data.aws_eks_cluster.eks_cluster.name
    "alpha.eksctl.io/nodegroup-name" = format("ng1-%s-experiment2", data.aws_eks_cluster.eks_cluster.name)
  }
  node_group_name = format("ng1-%s-experiment2", data.aws_eks_cluster.eks_cluster.name)
  node_role_arn   = data.terraform_remote_state.iam.outputs.nodegroup_role_arn
  #release_version = "1.17.11-20201007"
  subnet_ids = [
      data.terraform_remote_state.net.outputs.sub-priv1,
      data.terraform_remote_state.net.outputs.sub-priv2,
      data.terraform_remote_state.net.outputs.sub-priv3,
  ]
  tags = {
    "alpha.eksctl.io/cluster-name"                = data.aws_eks_cluster.eks_cluster.name
    "alpha.eksctl.io/eksctl-version"              = "0.29.2"
    "alpha.eksctl.io/nodegroup-name"              = format("ng1-%s-experiment2", data.aws_eks_cluster.eks_cluster.name)
    "alpha.eksctl.io/nodegroup-type"              = "managed"
    "eksctl.cluster.k8s.io/v1alpha1/cluster-name" = data.aws_eks_cluster.eks_cluster.name
  }
  #version = "1.17"

  launch_template {
    name    = aws_launch_template.lt-ng-experiment2.name
    version = "1"
  }

  scaling_config {
    desired_size = 2
    max_size     = 3
    min_size     = 1
  }

  lifecycle {
    ignore_changes = [scaling_config[0].desired_size]
  }

  timeouts {}
}
